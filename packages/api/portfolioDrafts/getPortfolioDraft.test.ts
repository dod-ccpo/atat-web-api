...
import { baseHandler, NO_PORTFOLIO_PATH_PARAM, NO_SUCH_PORTFOLIO } from "./getPortfolioDraft";
...
describe("Validation tests", () => {
  it("should require path param", async () => {
    ...
    expect(await baseHandler(emptyRequest)).toEqual(NO_PORTFOLIO_PATH_PARAM);
  });

  it("should return error if portfolio ID does not exist", async () => {
    const response: GetCommandOutput = {
      requestContext: { identity: { sourceIp: "10.2.2.2" } },
    } as any;
    ddbMock.on(GetCommand).resolves(response);
    expect(await baseHandler(validRequest)).toEqual(NO_SUCH_PORTFOLIO);
  });
...
describe("Successful operation tests", () => {
  ...
  it("should return Item", async () => {
    const result = await baseHandler(validRequest);
