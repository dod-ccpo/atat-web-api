...
describe("Validation tests", () => {
  it("should require path param", async () => {
    ...
    expect(await handler(emptyRequest, {} as Context, () => null)).toEqual(NO_PORTFOLIO_PATH_PARAM);
  });
  ...
  it("should return error if portfolio ID does not exist", async () => {
    ...
    expect(await handler(validRequest, {} as Context, () => null)).toEqual(NO_SUCH_PORTFOLIO);
  });
