...
import { AtatApiError, HothProvisionRequest, IAtatClient, ProvisionCspResponse } from "../client";
import * as atatApiTypes from "../client/types";
import { makeClient } from "../../utils/atat-client";
import { provisionRequestNoPayloadSchema } from "../../models/provisioning-schemas";
...
async function makeRequest(client: IAtatClient, request: HothProvisionRequest): Promise<ProvisionCspResponse> {
  // This function will always be operating for getting  portfolios; if we have something
  // else, this will be a non-recoverable error anyway.
  if (!request.portfolioId) {
    return {
      code: 404,
      content: {
        response: { details: "InvalidPortfolioId" },
        request: { portfolioId: request.portfolioId ?? "" } as atatApiTypes.GetPortfolioRequest,
      },
    };
  }

  const getPortfolioRequest: atatApiTypes.GetPortfolioRequest = {
    portfolioId: request.portfolioId,
  };

  try {
    logger.info(`Invoking getPortfolioById against CSP ${request.targetCspName}`);
    const cspResponse = await client.getPortfolioById(getPortfolioRequest);
    return transformSynchronousResponse(cspResponse, getPortfolioRequest, request);
  } catch (err) {
...
export const handler = middy(baseHandler)
  ...
  .use(validatorMiddleware({ eventSchema: transpileSchema(provisionRequestNoPayloadSchema) }))
